...
| [유튜브 추출](./ytdl/) | 유튜브(및 yt-dlp 지원 사이트) 영상/오디오를 화질 선택·구간 자르기·자막과 함께 추출하는 웹앱. 오디오만 mp3로도 추출. 재생목록은 곡 선택 후 일괄 mp3(320k, 태그·앨범아트 포함)로 zip. ffmpeg 기반. | `uvicorn app:app` | 광고·악성 팝업 범벅 다운로더 사이트, 유료 유튜브 다운로드 서비스 |

### 개발

| 도구 | 설명 | 실행 | 대체 |
|------|------|------|------|
| [개발 도구 모음](./devkit/) | JSON 정리·검증, Base64, URL 인코딩, JWT 디코드, 해시(SHA), 타임스탬프, UUID, 비밀번호 생성, 색상 변환(HEX/RGB/HSL), 정규식 테스터, 텍스트 도구. 전부 브라우저에서 처리해 데이터가 서버로 안 나감(프라이버시). | `uvicorn app:app` | 광고·추적 범벅 온라인 개발 유틸 사이트 (민감정보 붙여넣기 불필요) |
